...
- (void)handleAutoStopTimer:(NSTimer *)theTimer;
...
@property (retain) NSTimer *autoStopTimer;
...
@synthesize autoStopValue;
@synthesize minShowTime;
@synthesize autoStopTimer;
...
- (void)setMode:(ProgressSpinnerMode)newMode 
{
...
- (void)setProgress:(float)newProgress 
{
    ...
    if (mode == ProgressSpinnerModeDeterminate) 
    {
		if ([NSThread isMainThread]) 
        {
			[self updateProgress];
...
- (void)updateProgress 
{
    [(PSRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators 
{
    ...
    if (mode == ProgressSpinnerModeDeterminate) 
    {
        self.indicator = [[[PSRoundProgressView alloc] initWithDefaultSize] autorelease];
    }
    else 
    if (mode == ProgressSpinnerModeCustomView && self.customView != nil){
...
{
	ProgressSpinner *theSpinner = [[ProgressSpinner alloc] initWithView:view];
	[view addSubview:theSpinner];
	[theSpinner show:animated];
	return [theSpinner autorelease];
}
...
+ (BOOL)hideProgressSpinnerForView:(UIView *)view animated:(BOOL)animated 
{
    ...
    {
		if ([v isKindOfClass:[ProgressSpinner class]]) 
        {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) 
    {
		ProgressSpinner *theSpinner = (ProgressSpinner *)viewToRemove;
		theSpinner.removeFromSuperViewOnHide = YES;
		[theSpinner hide:animated];
		return YES;
	} 
    else 
    {
...
- (id)initWithView:(UIView *)view 
{
	...
	if (!view) 
    {
		[NSException raise:@"ProgressSpinnerViewIsNillException" 
					format:@"The view used in the ProgressSpinner initializer is nil."];
	}
    
// Fixed jumping spinner problem
//    
    UIScreen *screen = [UIScreen mainScreen];
	id me = [self initWithFrame: screen.bounds];
    
//	id me = [self initWithFrame:view.bounds];
	// We need to take care of rotation ourselfs if we're adding the Progress Spinner to a window
...
- (id)initWithFrame:(CGRect)frame 
{
    if ((self = [super initWithFrame:frame])) 
    {
        ...
        self.animationType = ProgressSpinnerAnimationFade;
        self.mode = ProgressSpinnerModeIndeterminate;
        self.labelText = nil;
...
- (void)layoutSubviews 
{
    ...
    // Compute Progress Spinner dimensions based on indicator size (add margin to Progress Spinner border)
...
- (void)show:(BOOL)animated 
{
	...
	// If the auto stop timer is set postpone the Progress Spinner display
	if (self.autoStopValue > 0.0) 
    {
		self.autoStopTimer = [NSTimer scheduledTimerWithTimeInterval:self.autoStopValue 
														   target:self 
														 selector:@selector(handleAutoStopTimer:) 
														 userInfo:nil 
														  repeats:NO];
	} 
	// ... otherwise show the Progress Spinner imediately 
	else 
    {
		[self setNeedsDisplay];
...
- (void)hide:(BOOL)animated 
{
	...
	// If the minShow time is set, calculate how long the Progress Spinner was shown,
	...
	// ... otherwise hide the Progress Spinner immediately
    [self hideUsingAnimation:useAnimation];
}

- (void)handleAutoStopTimer:(NSTimer *)theTimer 
{
	// Show the Progress Spinner only if the task is still running
	if (taskInProgress) 
    {
		[self setNeedsDisplay];
		[self showUsingAnimation:useAnimation];
...
{
    self.alpha = 0.0;
    if (animated && animationType == ProgressSpinnerAnimationZoom) 
    {
    ...
    if (animated) 
    {
        ...
        self.alpha = 1.0;
        if (animationType == ProgressSpinnerAnimationZoom) 
        {
...
- (void)hideUsingAnimation:(BOOL)animated 
{
    ...
    if (animated) 
    {
        ...
        // 0.02 prevents the Progress Spinner from passing through touches during the animation the Progress Spinner will get completely hidden
        // in the done method
        if (animationType == ProgressSpinnerAnimationZoom) 
        {
...
- (void)drawRect:(CGRect)rect 
{
    ...
    // Draw rounded Progress Spinner background rect
...
@implementation PSRoundProgressView
